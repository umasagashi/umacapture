...
15. [プレビューと保存・エラー表示](#15-プレビューと保存エラー表示)
...
4. **「評価」**ボタンでプレビューします（先頭の数件に対して実行し、結果と平均実行時間を表示）。
5. 問題なければ保存します。コンパイルエラー・実行エラー・処理が重すぎる場合は保存できません（[15 章](#15-プレビューと保存エラー表示)）。
...
`icon` には次の名前（文字列）が使えます。`null` を渡すとアイコン無しです。

`star` `star_border` `check` `check_circle` `close` `cancel` `warning` `error` `info`
`flag` `bolt` `favorite` `circle` `square` `arrow_upward` `arrow_downward`
`trending_up` `trending_down` `lock` `verified`
...
- **「評価」ボタン**：先頭の数件（約 20 件）に対して実際に実行し、各行の表示／表示可否／色や、
  **平均実行時間**を表示します。書きながら結果を確認できます。
- **コンパイルエラー**（文法ミスなど）：列全体がエラー表示（`⚠`）になり、行は消えません。エラー箇所が表示されます。
