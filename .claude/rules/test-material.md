...
- `testdata/clips/grid/` — the encode / scale regression grid's material.
...
- `testdata/harness/` — the live-capture harness's scratch data root (`appdrive_root/`) and its
  per-run artefacts (`runs/`), plus `firefox-live-capture/` — the web live-capture playtest checklist and its baseline measurements, filed here rather than under `evidence/` because it is a procedure to follow, not a record of a past measurement. The run artefacts are measurements; earlier runs are what a new
  run is compared against, so `runs/` is never emptied.
- `testdata/evidence/` — primary material that a shipped constant or a source comment cites as
  its derivation. Named by dropping the `analysis/` level: `.notes/analysis/<X>/` → `testdata/evidence/<X>/`.
